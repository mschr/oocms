...
function filterChildArray(&$files) {
	global $deep;
	global $query;
	global $patterns;
	global $ignoreCase;
	global $rootDir;
	$filtered = array();
	if (!$deep && is_array($query) && isset($query['name'])) {
		// if top pull (no path param)
		if (isset($files[0]))
			for ($i = 0; $i < count($files); $i++) {
				...
				if ($file['directory']) {
					...
					for ($offset = 0; $offset < count($file['children']); $offset++) {

						$fullPath = realPath(realPath($rootDir) . "/" . $file['path']) .
								  "/" . $file['children'][$offset];
						if (is_array($file['children'][$offset])
								  || is_dir($fullPath)
								  || preg_match("%{$patterns["name"]}%", $file['children'][$offset])) {
							$filtered[] = $file['children'][$offset];
						}
					}
					$files[$i]['children'] = $filtered;
				}
			}
		// only one file (path param isset)
		else if ($files['directory']) {

			for ($offset = 0; $offset < count($files['children']); $offset++) {

				$fullPath = realPath(realPath($rootDir) . "/" . $files['path']) .
						  "/" . $files['children'][$offset];

				if (is_array($files['children'][$offset])
						  || is_dir($fullPath)
						  || preg_match("%" . patternToRegExp($query['name']) . "%", $files['children'][$offset])) {
					$filtered[] = $files['children'][$offset];
				}
			}
			$files['children'] = $filtered;
		}
	}
}
...
function generateFileObj($file, $dir, $rootDir, $expand, $showHiddenFiles) {
	...
	if (is_dir($fullPath)) {
		$children = array();
		if (isset($_GET['AddDotDotDir'])) {
			$children[] = array(
				 "name" => "&lsaquo; &lsaquo; &lsaquo; &lsaquo;",
				 "parentDir" => $dir,
				 "directory" => true,
				 "path" => "..",
				 "size" => "1337",
			...
			if ($cFile) {
				if ($cFile != ".." && $cFile != ".") {
					if ($showHiddenFiles || $cFile[0] != '.') {
						if (!$expand) {
							if (is_dir($resolvedFullPath . "/" . $cFile))
								$_directories[] = $cFile;
							else
								$_files[] = $cFile;
						} else {
