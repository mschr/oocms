	...
	"dijit/Tree"
		...
		var Tree = declare("OoCmS._treebase", [dijit.Tree], {
			...
			constructor: function(args) {
				...
				console.info("_treebase.constructor"); //traceLog(this,arguments));
				var params = args || {};
...
//				console.info(traceLog(this,arguments));
			...
			getTooltipContents: function(item) {
				...
				if(!/file|dir/.test(item.type))
					for(var key in item) {
						if(/^_/.test(key) || item[key] == "")
							...
							continue;
						else if(/file|dir/.test(item.type) && /type|id|title|icon/.test(key))
							continue;
				...
				else if(item.id < 9995) {
					var store = this.tree.model.store
					if(/file/.test(item.type)) {
